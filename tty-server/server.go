...
func NewTTYServer(config TTYServerConfig) (server *TTYServer) {
	...
	app.Init()
	routesHandler.HandleFunc("/logout", logout.LogoutHandler)
	routesHandler.HandleFunc("/callback", callback.CallbackHandler)
	routesHandler.HandleFunc("/login", login.LoginHandler)
	//routesHandler.HandleFunc("/", home.HomeHandler)
	routesHandler.HandleFunc("/login", login.LoginHandler)
	routesHandler.HandleFunc("/logout", logout.LogoutHandler)
	routesHandler.HandleFunc("/callback", callback.CallbackHandler)
	routesHandler.Handle("/user", negroni.New(
			negroni.HandlerFunc(middlewares.IsAuthenticated),
			negroni.Wrap(http.HandlerFunc(user.UserHandler)),
	))
	routesHandler.PathPrefix("/public/").Handler(http.StripPrefix("/public/", http.FileServer(http.Dir("public/"))))
	//http.Handle("/", r)
